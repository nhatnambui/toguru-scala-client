...
object RemoteActivationsProvider {
  ...
  def apply(
      ...
      sttpBackend: SttpBackend[Identity, Nothing, NothingT] = HttpURLConnectionBackend()
...
class RemoteActivationsProvider(
  ...
  private val schedule: ScheduledFuture[_] = executor.scheduleAtFixedRate(
    new Runnable() {
      def run(): Unit = update()
    },
  ...
  private[toguru] def fetchToggleStates(sequenceNo: Option[Long]): Option[ToggleStates] = {
    ...
    def sequenceNoValid(toggleStates: ToggleStates): Boolean =
      (sequenceNo, toggleStates.sequenceNo) match {
        case (None, _)          => true
